...
import Weather exposing (Msg, Weather)
...
createWeatherLi ( maybeWeather, number ) =
    case maybeWeather of
        Nothing ->
            li [ getWeatherStyle False ] [ String.fromInt number ++ ":" ++ "Empty weather" |> text ]

        Just weather ->
            li [ getWeatherStyle True ] [ ul [] (prettyPrintWeather ( weather, number )) ]
...
prettyPrintWeather ( weather, number ) =
    List.map (\header -> header ++ ":") [ "Number", "Moisture", "Temperature", "Cloudy" ]
        |> List.map2 combineValueWithHeader
            [ number
            , weather.moisture
            , weather.temperature
